...
$os                     = "ubuntu"
$num_instances          = 3
$instance_name_prefix   = "hdfs"
$vm_memory              = 2048
$vm_cpus                = 1
$subnet                 = "10.0.5.1" # For 10.0.5.1X
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "hdfs" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"],
            "hdfs_master" => ["#{$instance_name_prefix}01"],
            "hdfs_slave" => ["#{$instance_name_prefix}0[2:#{$num_instances}]"]
          }
        end
      end
    end
